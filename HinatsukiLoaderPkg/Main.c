...
EFI_STATUS SaveMemoryMap(struct MemoryMap *map, EFI_FILE_PROTOCOL *file) {
  ...
  for (iter = (EFI_PHYSICAL_ADDRESS)map->buffer, i = 0;
    ...
    EFI_MEMORY_DESCRIPTOR *desk = (EFI_MEMORY_DESCRIPTOR *)iter;
    len = AsciiSPrint(buf, sizeof(buf), "%u, %x, %-ls, %08lx, %lx, %lx\n", i,
                      desk->Type, GetMemoryTypeUnicode(desk->Type),
                      desk->PhysicalStart, desk->NumberOfPages,
                      desk->Attribute & 0xffffflu);
...
CHAR16 *GetPixelFormatUnicode(EFI_GRAPHICS_PIXEL_FORMAT fmt) {
  switch (fmt) {
  case PixelBitMask:
...
EFI_STATUS EFIAPI UefiMain(EFI_HANDLE image_handle,
  ...
  if (EFI_ERROR(status)) {
    Print(L"Failed to close memory map file: %r\n", status);
    Halt();
  }
  ...
  case PixelBlueGreenRedReserved8BitPerColor:
    ...
    break;
  default:
    Print(L"Unspported pixel format: %d\n", gop->Mode->Info->PixelFormat);
    Halt();
  }
